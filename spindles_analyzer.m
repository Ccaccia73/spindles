...
% Last Modified by GUIDE v2.5 18-Feb-2013 15:38:17
...
if (handles.actBehavior >= 1 && handles.actBehavior <= 8)
    ...
    if (~isnan(val) && imag(val) == 0 && mod(val,1) == 0 && val >= 0 )
        handles.spindles_detect{handles.actBehavior}(handles.currIndex) = val;
        guidata(hObject,handles);
        updateSpindles(handles);
		set(handles.status_text, 'String','Spindles value OK');
	else
		set(handles.status_text, 'String','NOT a valid number for spindles count');
		updateSpindles(handles);
    end
end
...
set(handles.spindlesCount,'String',num2str(currVal));
